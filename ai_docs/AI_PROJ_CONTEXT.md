...
## Project Overview
- **Project Name**: PanelTracker
- **Project Type**: local, lightweight patient panel management app
- **Primary Goal**: To increase patient follow-up compliance by 75% within the first year of deployment. To decrease the physician time spent on chart review and follow-ups by 30% per week within the first month of deployment.
- **Target Completion**: end of Q1 2026

## Core Features

1.  **Patient & Panel Management**: Core CRUD functionality for patients, problems, medications, labs, etc., as detailed in the existing documentation.
2.  **Dashboard & Reminders**: At-a-glance view of critical patient information, including overdue tasks, medication refills, and pending labs.
3.  **Voice-Powered Data Entry (NEW)**:
    - **Functionality**: A key feature where a provider can tap a button and use voice commands to perform complex actions.
    - **Example Command**: "add patient. Laura Jones. 45 female. G4P0030. on atorvastatin 40 BID, aspirin 81 mg daily. pregnant. married to James Jones. birthday 24 december 1998. follow up regarding blood pressure next week. pressure today was 145 over 89. blood glucose 100. referral out to MFM."
    - **Expected Outcome**: The app should parse this natural language command to:
        - Create a new patient ("Laura Jones").
        - Populate demographics (age, sex, DOB).
        - Add medications ("atorvastatin", "aspirin").
        - Add to the problem list ("pregnant").
        - Link to a family member ("James Jones").
        - Set reminders ("follow up regarding blood pressure").
        - Log vitals/labs ("blood pressure", "blood glucose").
        - Create a referral ("MFM").
    - **Implementation Note**: This will require integrating a small, efficient, local Large Language Model (LLM) capable of speech-to-text and intent recognition.
...
- **APIs**: Future possibility of connecting to external APIs to pull USPSTF guidelines or vaccine schedules.
- **Authentication**: None, as it is a local-only application.
- **Third-party Libraries**:
  - `vis.js` for family trees (existing).
  - New libraries will be needed for elegant, clean, and modern data visualizations.
- **Data Sources**: None, as data is local.
- **LLM for Speech-to-Text**: Integration with a small, local Large Language Model for voice commands and data entry (see Core Features).


## Project-Specific Rules

### 1. Code Style & Standards
- **Overall Principle**: Maintain the current lightweight, single-file structure for simplicity and speed. The application must remain "light and snappy" and run easily on Linux.
- **Python (app.py)**:
  - **Style Guide**: Strictly follow PEP 8. Use a linter like `flake8` or a formatter like `black` to ensure consistency.
  - **Naming Conventions**: Use `snake_case` for all variables and functions.
  - **File Organization**: Keep the backend self-contained within `app.py`. If complexity grows, we can consider moving to a "blueprints" pattern, but not before.
  - **Comment Style**: Use docstrings for all functions. Use `── Section Name ──` style comments to delineate logical sections of the file.
- **JavaScript (static/js/app.js)**:
  - **Style Guide**: Use modern vanilla JavaScript (ES6+). Avoid introducing frameworks to keep it lightweight.
  - **Naming Conventions**: Use `camelCase` for variables and functions. The utility functions `$` and `$$` are established for `querySelector` and `querySelectorAll`, respectively.
  - **File Organization**: Keep frontend logic within `static/js/app.js`.
  - **Comment Style**: Use `// ── Section Name ──` comments to organize the file. Add JSDoc-style comments for all new functions to clarify parameters and return values.
- **CSS (static/css/style.css)**:
  - **Style Guide**: Continue using CSS custom properties (`--var`) for theming.
  - **Naming Conventions**: Use descriptive, hyphenated class names (e.g., `patient-header`, `stat-card`).
- **SQL (schema.sql)**:
  - **Style Guide**: Write all SQL keywords (e.g., `CREATE TABLE`, `INTEGER`) in uppercase for readability.
  - **Naming Conventions**: Use `snake_case` for all table and column names.
- **Editable Clinical Values**: Key clinical values such as lab ranges, blood pressure ranges, etc., should be hard-coded for initial setup but easily editable by the end-user without requiring code changes.


### 2. Development Constraints
- **Overall Principle**: Design should prioritize efficiency and speed of use.
- **Performance Targets**: The app should be responsive for a panel of 100-2500 patients.
- **Browser/Platform Support**: Runnable in any browser from 2013 onwards.
- **Security Requirements**: Local-only tool. Database encryption will be considered later.
- **Accessibility Standards**: Accessibility is not a primary concern for this application, as it targets an elite group of users. Efficiency and speed of use take precedence.

## Project-Specific Workflows

### Development Process
1. **Local Development**:
   - `python3 seed.py` (optional)
   - `python3 app.py`
   - The app will be running at http://localhost:5000

2. **Testing Requirements**:
   - No formal tests are in place, but the code should be written with testability in mind.

3. **Build & Deployment**:
   - No build step.
   - Deployment is running the python app locally.

### Git Workflow
- **Branch Naming**: feature/[feature-name]
- **Commit Messages**: Conventional Commits
- **PR Process**: Create a pull request for every new feature.
- **Code Review**: At least one other person should review the PR.

---

## Project Specific Documentation from CLAUDE.md
